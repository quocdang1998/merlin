...
#include "ap3_mpo/hdf5_utils.hpp"  // merlin::ext::ap3mpo::ls_groups, merlin::ext::ap3mpo::check_string_in_array
                                   // merlin::ext::ap3mpo::get_dset, merlin::ext::ap3mpo::append_suffix
                                   // merlin::ext::ap3mpo::find_element
...
// Constructor
ext::ap3mpo::Ap3HomogXS::Ap3HomogXS(const std::string & filename,
...
mpo_file_(filename.c_str(), H5F_ACC_RDONLY) {
    ...
    this->geometry_ = ext::ap3mpo::Ap3Geometry(geometry_id, this->mpo_file_);
    this->energymesh_ = ext::ap3mpo::Ap3EnergyMesh(energy_mesh_id, this->mpo_file_);
    this->isotope_ = ext::ap3mpo::Ap3Isotope(isotope, this->mpo_file_);
    this->reaction_ = ext::ap3mpo::Ap3Reaction(reaction, this->mpo_file_);
    this->state_param_ = ext::ap3mpo::Ap3StateParam(this->mpo_file_);
    // get ouput id
    H5::Group outputs = this->mpo_file_.openGroup("output");
    auto [output_ids, _shape] = ext::ap3mpo::get_dset<int>(&outputs, "OUPUTID");
    int output_id = output_ids[ndim_to_contiguous_idx({this->geometry_.index, this->energymesh_.index}, _shape)];
    std::string output_id_str = ext::ap3mpo::append_suffix("output_", output_id);
    std::printf("    Got output ID: %s.\n", output_id_str.c_str());
    this->output_ = outputs.openGroup(output_id_str.c_str());
}

// Merge 2 objects
ext::ap3mpo::Ap3HomogXS & ext::ap3mpo::Ap3HomogXS::operator+=(ext::ap3mpo::Ap3HomogXS & other) {
    ...
    std::list<ext::ap3mpo::Ap3HomogXS *> old_linked_instances = this->linked_instances_;
    this->linked_instances_.clear();
    std::set_union(old_linked_instances.begin(), old_linked_instances.end(),
                   other.linked_instances_.begin(), other.linked_instances_.end(),
                   std::back_inserter(this->linked_instances_));
    this->linked_instances_.push_back(&other);
    return *this;
}

// Get shape of parameter space
intvec ext::ap3mpo::Ap3HomogXS::get_output_shape(void) {
    intvec result(this->state_param_.param_names.size()+2);
...
// Assign Stock
void ext::ap3mpo::Ap3HomogXS::assign_destination_array(array::NdData & dest) {
    this->pdata_ = &dest;
    for (ext::ap3mpo::Ap3HomogXS * linked : this->linked_instances_) {
        linked->assign_destination_array(dest);
    }
}

// Get microscopic homogenized cross-section to stock
void ext::ap3mpo::Ap3HomogXS::write_to_stock(const ext::ap3mpo::Ap3StateParam & pspace, const std::string & xstype) {
    // check xstype
    static std::vector<std::string> allowed_xstype = {"micro", "macro", "zoneflux", "RR"};
    std::uint64_t xs_type_index = ext::ap3mpo::check_string_in_array(xstype, allowed_xstype);
    ...
    auto [addrxs, addrxs_shape] = ext::ap3mpo::get_dset<int>(&(this->output_), "info/ADDRXS");
    // get index of isotope and reaction
    auto [i_iso_in_geo, num_iso_in_geo] = ext::ap3mpo::get_dset<int>(&(this->output_), "info/ISOTOPE");
    std::uint64_t i_iso = ext::ap3mpo::find_element(i_iso_in_geo, this->isotope_.index);
    if (i_iso == UINT64_MAX) {
        WARNING("Isotope not found in the geometry, doing nothing.\n");
        return;
    }
    std::printf("    Found isotope in geometry (index %" PRIu64 ").\n", i_iso);
    auto [i_reac_in_geo, num_reac_in_geo] = ext::ap3mpo::get_dset<int>(&(this->output_), "info/REACTION");
    std::uint64_t i_reac = ext::ap3mpo::find_element(i_reac_in_geo, this->reaction_.index);
    ...
    std::vector<std::string> statepts = ext::ap3mpo::ls_groups(&(this->output_), "statept_");
    ...
    for (const std::string & statept_str : statepts) {
        ...
        std::uint64_t statept_idx = ext::ap3mpo::get_suffix(statept_str, "statept_");
        std::printf(" %" PRIu64, statept_idx);
        // calculate index of parameters wrt the storing array
        intvec stock_index(pspace.param_names.size()+2);
        auto [paramvalueord, paramvalueord_shape] = ext::ap3mpo::get_dset<int>(&statept, "PARAMVALUEORD");
        ...
        for (int i_param = 0; i_param < pspace.param_names.size(); i_param++) {
            ...
            std::uint64_t i_paramvalueord = ext::ap3mpo::find_element(this->state_param_.param_names, param_name);
            double & param_value = this->state_param_.param_values[param_name][paramvalueord[i_paramvalueord]];
            std::uint64_t i_param_in_stock = ext::ap3mpo::find_element(pspace.param_values.at(param_name), param_value);
            stock_index[i_param+1] = i_param_in_stock;
        }
        // loop on each zone
        std::vector<std::string> zones = ext::ap3mpo::ls_groups(&statept, "zone_");
        ...
        for (const std::string & zone_str : zones) {
            ...
            std::uint64_t zone_idx = ext::ap3mpo::get_suffix(zone_str, "zone_");
            // get isotope concentration
            auto [concentrations, concentrations_shape] = ext::ap3mpo::get_dset<float>(&zone, "CONCENTRATION");
            concentration[zone_idx] = concentrations[i_iso];
            // get zone flux
            auto [zonefluxs, zoneflux_shape] = ext::ap3mpo::get_dset<float>(&zone, "ZONEFLUX");
            ...
            auto [addrzx_data, _] = ext::ap3mpo::get_dset<int>(&zone, "ADDRZX");
            std::uint64_t addrzx = addrzx_data[0];
            int crossection_index = addrxs[ndim_to_contiguous_idx(intvec({addrzx, i_iso, i_reac}), addrxs_shape)];
            auto [crossections, crossections_shape] = ext::ap3mpo::get_dset<float>(&zone, "CROSSECTION");
