...
int main(void) {
    ...
    merlin::intpl::SparseGrid grid({{0.0, 1.0, 2.0}, {0.0, 1.0, 2.0}}, 1, merlin::intvec({1, 1}));
    ...
    merlin::intpl::PolynomialInterpolant plm_int(grid, value, merlin::intpl::Method::Lagrange);
    MESSAGE("Coefficients are: %s\n", plm_int.get_coeff().str().c_str());

    merlin::intpl::CartesianGrid c_grid({{0.0, 1.0, 2.0}, {0.0, 1.0, 2.0}});
