...
void test_householder(void) {
    double matrix_data[9] = {12, 6, -4, -51, 167, 24, 4, -68, -41};
    linalg::Matrix mat(matrix_data, {3, 3});
    MESSAGE("Initial matrix: %s\n", mat.str().c_str());

    // test 1st Householder reflection
    floatvec reflector({-1.f / std::sqrt(14), 3.f / std::sqrt(14), -2.f / std::sqrt(14)});
    for (std::uint64_t i = 0; i < 3; i++) {
        linalg::householder(reflector.data(), matrix_data + 3*i, 3, 3);
    }
    ...
    for (std::uint64_t i = 0; i < 2; i++) {
        linalg::householder(reflector.data() + 1, matrix_data + 4 + 3*i, 2, 2);
    }
