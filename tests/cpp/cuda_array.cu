...
__global__ void print_gpu_array(double * gpu_data) {
...
int main(void) {
    ...
    unsigned int strides[2] = {5*sizeof(double), 2*sizeof(double)};
    ...
    for (merlin::Array<double>::iterator it = A.begin(); it != A.end(); it++) {
