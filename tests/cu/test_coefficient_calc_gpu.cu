...
int main(void) {
    ...
    merlin::interpolant::PolynomialInterpolant plm_int(grid, coeff, merlin::interpolant::Method::Newton, stream, 32);
    stream.synchronize();
    MESSAGE("Result GPU: %s\n", plm_int.get_coeff().str().c_str());
    merlin::interpolant::PolynomialInterpolant plm_int_cpu(grid, value_cpu, merlin::interpolant::Method::Newton);
    MESSAGE("Result CPU: %s\n", plm_int_cpu.get_coeff().str().c_str());

    merlin::array::Array points(merlin::intvec({1,value_cpu.ndim()}));
    points[{0,0}] = 2.2; points[{0,1}] = 1.2; points[{0,2}] = 2.0;
