...
int main(void) {
    ...
    merlin::intpl::PolynomialInterpolant plm_int(grid, coeff, merlin::intpl::Method::Newton, stream, 32);
    stream.synchronize();
    MESSAGE("Result GPU: %s\n", plm_int.get_coeff().str().c_str());
    merlin::intpl::PolynomialInterpolant plm_int_cpu(grid, value_cpu, merlin::intpl::Method::Newton);
