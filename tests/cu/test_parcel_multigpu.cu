...
int main(void) {
    ...
    Message("First Parcel object: {}\n", B.str());
    Message("Current GPU ID: {}\n", cuda::Device::get_current_gpu().str());

    // copy data to the same GPU
    array::Parcel B_(B);
    Message("Parcel object copied to the same GPU: {}\n", B_.str());
    Message("Current GPU ID: {}\n", cuda::Device::get_current_gpu().str());
    ...
    array::Parcel B1(B);
    Message("Parcel object copied to another GPU: {}\n", B1.str());
    Message("Current GPU ID: {}\n", cuda::Device::get_current_gpu().str());
}
