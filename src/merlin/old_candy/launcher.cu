...
p_model_(p_model), p_data_(p_train_data), p_optimizer_(p_optimizer), n_thread_(n_thread), model_size_(model_size),
shared_mem_(share_mem) {
    ...
    // allocate gradient data on GPU
    ::cudaMallocAsync(&this->gradient_data_, this->model_size_ * sizeof(double),
                      reinterpret_cast<::cudaStream_t>(p_stream->get_stream_ptr()));
...
candy::Launcher::~Launcher(void) {
    ...
    if (this->gradient_data_ != nullptr) {
        if (this->is_gpu()) {
            ::cudaFreeAsync(this->gradient_data_, reinterpret_cast<::cudaStream_t>(stream_ptr->get_stream_ptr()));
        } else {
