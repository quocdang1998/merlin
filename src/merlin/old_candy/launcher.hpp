...
class candy::Launcher {
  public:
    ...
    /** @brief Constructor from a model and an array pre-allocated on GPU.*/
    MERLIN_EXPORTS Launcher(candy::Model * p_model, const array::Parcel * p_train_data, candy::Optimizer * p_optimizer,
                            std::uint64_t model_size, std::uint64_t share_mem, std::uint64_t n_thread = 1);
  ...
  private:
    ...
    /** @brief Size of model.*/
    std::uint64_t model_size_;
    /** @brief Size of shared memory.*/
    std::uint64_t shared_mem_ = 0;
};
...
std::future<void> * cpu_async_launch(candy::Model * p_model, const array::Array * p_train_data,
                                     ...
                                     std::uint64_t rep, std::uint64_t n_thread);

/** @brief Launch asynchronously model fitting algorithm on GPU.*/
void gpu_async_launch(candy::Model * p_model, const array::Parcel * p_train_data, candy::Optimizer * p_optimizer,
                      double * p_gradient, std::uint64_t model_size, std::uint64_t rep, std::uintptr_t stream_ptr,
