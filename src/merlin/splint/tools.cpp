...
void splint::construct_coeff_cpu(std::future<void> && current_job, double * coeff, const grid::CartesianGrid * p_grid,
                                 const Index * p_method, std::uint64_t n_threads) noexcept {
    ...
    _Pragma("omp parallel num_threads(n_threads)") {
        ...
        for (std::uint64_t i_dim = 0; i_dim < p_grid->ndim(); i_dim++) {
            ...
            const std::uint64_t & i_method = (*p_method)[i_dim];
...
void splint::construct_coeff_gpu(double * coeff, const grid::CartesianGrid * p_grid, const Index * p_method,
...
void splint::eval_intpl_cpu(std::future<void> && current_job, const double * coeff, const grid::CartesianGrid * p_grid,
                            const Index * p_method, const double * points, std::uint64_t n_points, double * result,
        ...
        for (std::uint64_t i_point = thread_idx; i_point < n_points; i_point += n_threads) {
            ...
            do {
                splint::recursive_interpolate(coeff, p_grid->size(), contiguous_index, loop_index.data(), cache.data(),
                                              ...
                                              p_grid->grid_vectors().data(), *p_method, ndim);
                last_updated_dim = increment_index(loop_index.data(), p_grid->shape().data(), ndim);
                contiguous_index++;
...
void splint::eval_intpl_gpu(double * coeff, const grid::CartesianGrid * p_grid, const Index * p_method, double * points,
                            std::uint64_t n_points, double * result, std::uint64_t n_threads, std::uint64_t ndim,
