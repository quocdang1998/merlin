...
void * regpl::Polynomial::copy_to_gpu(regpl::Polynomial * gpu_ptr, void * coeff_data_ptr,
    ...
    std::uint64_t * term_idx_ptr = reinterpret_cast<std::uint64_t *>(order_ptr + this->ndim());
    copy_on_gpu.term_idx_.data() = term_idx_ptr;
    copy_on_gpu.term_idx_.size() = this->size();
    copy_on_gpu.full_n_ = this->full_n_;
    ...
    ::cudaMemcpyAsync(order_ptr, this->order_.data(), this->ndim() * sizeof(std::uint64_t), ::cudaMemcpyHostToDevice,
                      stream);
    ::cudaMemcpyAsync(term_idx_ptr, this->term_idx_.data(), this->size() * sizeof(std::uint64_t),
    ...
    copy_on_gpu.term_idx_.data() = nullptr;
    return reinterpret_cast<void *>(term_idx_ptr + this->size());
}
