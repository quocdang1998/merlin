...
Iterator::Iterator(const intvec & index, const intvec & shape)  : index_(index), pshape_(&shape) {
    this->item_ptr_ = ndim_to_contiguous_idx(index, shape);
}

// Constructor from C-contiguous index
Iterator::Iterator(std::uint64_t index, const intvec & shape) : item_ptr_(index), pshape_(&shape) {
...
Iterator & Iterator::operator++(void) {
    ...
    while (this->index_[current_dim] >= (*(this->pshape_))[current_dim]) {
        if (current_dim == 0) {
            if (this->index_[current_dim] == (*(this->pshape_))[current_dim]) {
                break;
            } else {
    ...
    this->item_ptr_ = ndim_to_contiguous_idx(this->index_, *(this->pshape_));
    return *this;
}
...
void Iterator::update(void) {
    ...
    for (int i = this->index_.size() - 1; i >= 0; i--) {
        if (this->index_[i] >= (*(this->pshape_))[i]) {
            current_dim = i;
            break;
        }
    }
    ...
    while (this->index_[current_dim] >= (*(this->pshape_))[current_dim]) {
        ...
        ldiv_t carry = ldiv(static_cast<std::int64_t>(this->index_[current_dim]),
                            static_cast<std::int64_t>((*(this->pshape_))[current_dim]));
