...
#include "merlin/intpl/sparse_grid.hpp"
...
intvec intpl::SparseGrid::get_grid_shape(void) const noexcept {
...
std::uint64_t intpl::SparseGrid::size(void) const {
...
intvec intpl::SparseGrid::index_from_contiguous(std::uint64_t contiguous_index) const {
    // determine i_level
...
Vector<double> intpl::SparseGrid::point_at_index(const intvec & index) const {
...
bool intpl::SparseGrid::contains(const Vector<double> & point) const {
    ...
    std::uint64_t num_level = this->num_level();
    for (std::uint64_t i_level = 0; i_level < num_level; i_level++) {
        intpl::CartesianGrid cart_grid_level(intpl::get_cartesian_grid(*this, i_level));
...
void intpl::SparseGrid::add_points_to_grid(const Vector<double> & new_points, std::uint64_t dimension) {
...
// Representation
std::string intpl::SparseGrid::str(void) const {
...
intpl::CartesianGrid intpl::get_cartesian_grid(const SparseGrid & grid, std::uint64_t subgrid_index) {
    ...
    return intpl::CartesianGrid(std::move(cart_grid_vectors));
}
