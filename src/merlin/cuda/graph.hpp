...
/** @brief List of dependency nodes.*/
using GraphNodeList = vector::DynamicVector<cuda::GraphNode>;

/** @brief List of edges between two nodes.*/
using GraphEdgeList = vector::DynamicVector<std::array<cuda::GraphNode, 2>>;

/** Default beginning of graph.*/
MERLIN_EXPORTS extern const GraphNodeList begin_graphnode;
...
class cuda::Graph {
  public:
    ...
    /** @brief Get node list.*/
    MERLIN_EXPORTS cuda::GraphNodeList get_node_list(void) const;
     ...
     */
    MERLIN_EXPORTS cuda::GraphEdgeList get_edge_list(void) const;
    /// @}
     ...
     */
    template <typename Function, typename... Args>
    cuda::GraphNode add_host_node(Function && callback, const cuda::GraphNodeList & deps, Args &&... args);
#endif  // __NVCC__
