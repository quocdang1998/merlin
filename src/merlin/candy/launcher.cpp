...
// Calculate gradient and update gradient by relative loss function
static void update_gradient_relative(std::future<void> * current_job, candy::Model * p_model,
...
// Calculate gradient and update gradient by absolute loss function
static void update_gradient_absolute(std::future<void> * current_job, candy::Model * p_model,
                                     const array::Array * p_train_data, candy::Optimizer * p_optimizer,
                                     std::uint64_t model_size, std::uint64_t n_thread, std::uint64_t rep) {
    // synch the current job
    if (current_job != nullptr) {
...
std::future<void> * candy::cpu_async_launch(std::future<void> * current_job, candy::Model * p_model,
    ...
    switch (metric) {
        case candy::TrainMetric::RelativeSquare : {
            result = std::async(std::launch::async, update_gradient_relative, current_job, p_model, p_train_data,
                                p_optimizer, model_size, n_thread, rep);
            break;
        }
        case candy::TrainMetric::AbsoluteSquare : {
            result = std::async(std::launch::async, update_gradient_absolute, current_job, p_model, p_train_data,
                                p_optimizer, model_size, n_thread, rep);
            break;
        }
        default : {
            FAILURE(std::invalid_argument, "Unknown metric.\n");
        }
    }
...
void candy::gpu_asynch_launch(candy::Model * p_model, const array::Parcel * p_train_data,
                              ...
                              candy::TrainMetric metric, cuda::Stream * stream_ptr) {
...
void candy::Launcher::launch_async(std::uint64_t rep, candy::TrainMetric metric) {
