...
void train_by_cpu(std::shared_future<void> synch, candy::Model * p_model, array::Array * p_data,
                  ...
                  const std::function<bool(double, double)> * p_stop_condition, std::uint64_t n_threads,
                  intvec * p_cache_mem);
...
class candy::Trainer {
  public:
    ...
    /// @{
    /** @brief Get reference to the CP model.*/
    constexpr candy::Model & get_model(void) noexcept { return *(this->p_model_); }
    /** @brief Get constant reference to the CP model.*/
    constexpr const candy::Model & get_model(void) const noexcept { return *(this->p_model_); }
     ...
     *  @param stop_condition A function indicate when to stop the update process based on the mean square error. It
     *  takes errors before and after each step as argument, and stop the training process once this function returns
     *  ``false``.
     *  @param n_threads Number of parallel threads for training the model.
     *  @param metric Training metric for the model.
     */
    MERLIN_EXPORTS void update(std::uint64_t rep, const std::function<bool(double, double)> & stop_condition,
